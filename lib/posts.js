...
// カテゴリoutside
export async function getCategoryOutside(){
    const res = await fetch(new URL(`${API_ENDPOINT}/api/image/outside`));
    const posts = await res.json();
    return posts;
}
// カテゴリliving
export async function getCategoryLiving(){
    const res = await fetch(new URL(`${API_ENDPOINT}/api/image/living`));
    const posts = await res.json();
    return posts;
}